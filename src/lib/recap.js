...
const SIGNOFF = 'Generated by the Pit Boss app'
const SIGNOFF_H = 58
...
export function renderRecapCanvas(game, player, makeCanvas) {
  ...
  const top = 560
  const footer =
    (notes.length ? notes.length * 54 + 40 : 0) + (hidden ? 54 : 0) + SIGNOFF_H
  ...
  // Attribution, quiet enough that it never competes with the standings.
  g.fillStyle = '#6E635C'
  g.font = font(30, 500)
  g.fillText(SIGNOFF, M, fy + 12)
